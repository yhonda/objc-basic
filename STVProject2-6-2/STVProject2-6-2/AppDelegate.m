...
- (BOOL)application:(UIApplication *)app openURL:(NSURL *)url options:(NSDictionary<UIApplicationOpenURLOptionsKey,id> *)options {
    ...
    if ([scheme isEqualToString: AppSchmeName]) {
        ...
        secondViewController.receiveUrl = [NSString stringWithFormat:@"%@", url];
        secondViewController.receiveScheme = scheme;
        secondViewController.receiveQuery = [NSString stringWithFormat:@"%@", url.query];
        // 初回起動画面をセット
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"STVProject2_6_2"];
